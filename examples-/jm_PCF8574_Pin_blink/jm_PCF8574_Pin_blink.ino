...
#define __PROG__ "jm_PCF8574_Pin_blink"
...
jm_Pin PCF8574_LED(PCF8574.port_register(), 3, OUTPUT, false, true);
...
#ifdef __AVR__
	bool write_cgramP(uint8_t index, uint8_t count, const uint8_t font5x8P[]);
#endif
};
...
bool jm_LCM2004_I2C::write_cgram(uint8_t index, uint8_t count, const uint8_t font5x8[])
{
	if (!set_cgram_addr(index * 8)) return false;

	if (!write(font5x8, count * 8)) return false;
...
bool jm_LCM2004_I2C::write_cgramP(uint8_t index, uint8_t count, const uint8_t font5x8P[])
{
	if (!set_cgram_addr(index * 8)) return false;

	for (uint8_t i=0; i<count; i++)
		for (uint8_t j=0; j<8; j++)
			if (!write(pgm_read_byte_near((const uint8_t  *) font5x8P++))) return false;
...
const uint8_t custom_font5x8P[8][8] PROGMEM = {
...
const uint8_t custom_font5x8[8][8] = {
   { 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
   { 0x00, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
   { 0x00, 0x00, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00 },
   { 0x00, 0x00, 0x00, 0x1F, 0x00, 0x00, 0x00, 0x00 },
...
#ifdef __AVR__
	LCM2004_I2C.write_cgramP(0, 8, custom_font5x8P[0]);
#else
	LCM2004_I2C.write_cgram(0, 8, custom_font5x8[0]);
#endif
